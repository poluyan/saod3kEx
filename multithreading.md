...
Реализовать многопоточную версию подсчета простых чисел в списке с использованием одного из примитива синхронизации (mutex/atomic/semaphore). Например, поместив все числа из списка в очередь для проверки на простоту или создав две переменные, для текущего индекса и числа найденных простых чисел. Использование execution policy (`std::execution::par`) и parallel STL запрещается.
